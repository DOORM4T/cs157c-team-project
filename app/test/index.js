...
async function main() {
  ...
  const findInserted = await openflight.findOne({
  ...
  const updateResult = await openflight.updateOne(
    { _id: findInserted._id, airport_id: "test" },
    { $set: { airport_id: "test (updated)" } }
  );
  console.log(updateResult);

  // Query 3: Update a point of interest
  console.log("Q3: DELETING...");
  const deleteResult = await openflight.deleteOne({
    _id: findInserted._id,
    airport_id: "test (updated)",
  });
